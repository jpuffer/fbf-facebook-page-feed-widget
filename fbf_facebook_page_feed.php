...
<ul class="fbf_facebook_page_widget">';
			...
			if ($limit == 0) {
				...
				foreach ($blocks[$f] as $block) {
					...
					$like_button = '<iframe src="http://www.facebook.com/plugins/like.php?href='.$block->get_permalink().'&amp;layout=standard&amp;show_faces=true&amp;width=450&amp;action=like&amp;colorscheme=light&amp;height=30" scrolling="no" frameborder="0" style="border:none; overflow:hidden; height:30px;" allowTransparency="true"></iframe>';
					...
					if ($options['show_avatar'] != '') {
						...
						$feed_parent = $block->get_feed();
						$parent_url = $feed_parent->subscribe_url();
						$url_arr = explode("?id=",$parent_url);
						$url_qs = $url_arr[1];
					...
					if ($options['update'] != '') {
						...
						$time_markup = ''.sprintf( __('%s', 'rstw'),' <span class="facebook_page-timestamp"><abbr title="' .$block->get_date("j F Y, H:i:s") . '">'.timesince($time).'</abbr></span>' );
						
						$time_link ="<a href=".$block->get_permalink()." class='facebook_page-link' ".$link_target.">".$time_markup."</a>";
						
						$returnMarkup .= html_entity_decode($time_link,ENT_COMPAT, 'UTF-8');;
